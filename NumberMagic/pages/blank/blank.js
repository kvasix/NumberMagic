(function () {
    ...
    function checkShape(e1, e2) {
        ...
        if (true) {  // if we have a match, fill the numBox with white and show the status.
            ...
            if (!(--numpawnsleft)) {
                var score_post_array = [localSettings.values["sid"], 0, 0, "", ((hours * 60 + mins) * 60 + secs)];
                score_post(score_post_array).then(function () {
                    ...
                    if (mistakeCount > MISTAKE_THRESHOLD) {
                        ...
                        id('nextLevelBtn').addEventListener("click", function () { redirect_to_next_level(0) }, false);
                        id('nextLevelBtn').style.visibility = 'visible';
                    }

                    id('msgBox').innerHTML = message;
                    id('replayBtn').addEventListener("click", function () { redirect_to_next_level(-1) }, false);
